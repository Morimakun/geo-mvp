"""
Phase 4b Field Comparison with Phase 6B v3.1 Extraction Results

Reads phase6b_30pages_results_v3_1.json and outputs v3_1 comparison files.
"""
...
EXTRACTION_RESULT_PATH = PROJECT_ROOT / "data/test_outputs/phase6b_30pages_results_v3_1.json"
...
def main():
    print("=" * 100)
    print("PHASE 4B v3.1: FIELD COMPARISON WITH PHASE 6B v3.1 EXTRACTION")
    print("=" * 100)
    print()
    ...
    df_pages.to_csv(OUTPUT_DIR / "phase6b_v3_1_reconciliation_page_summary.csv", index=False, encoding='utf-8-sig')
    print(f"Saved: phase6b_v3_1_reconciliation_page_summary.csv ({len(df_pages)} rows)")

    df_fields = pd.DataFrame(all_field_comparisons)
    df_fields.to_csv(OUTPUT_DIR / "phase6b_v3_1_reconciliation_field_comparisons.csv", index=False, encoding='utf-8-sig')
    print(f"Saved: phase6b_v3_1_reconciliation_field_comparisons.csv ({len(df_fields)} rows)")

    df_mismatch = pd.DataFrame(all_mismatches)
    df_mismatch.to_csv(OUTPUT_DIR / "phase6b_v3_1_reconciliation_mismatch_list.csv", index=False, encoding='utf-8-sig')
    print(f"Saved: phase6b_v3_1_reconciliation_mismatch_list.csv ({len(df_mismatch)} rows)")
    ...
    with open(OUTPUT_DIR / "phase6b_v3_1_reconciliation_summary.json", 'w', encoding='utf-8') as f:
        json.dump(summary_json, f, ensure_ascii=False, indent=2, default=str)
    print(f"Saved: phase6b_v3_1_reconciliation_summary.json")
