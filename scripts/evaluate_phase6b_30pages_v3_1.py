"""
Phase 6B v3.1: Fix HI=40 regression from v3

Key change over v3:
  - existing_support: Strengthened template value blacklist (0, 15, 40 etc.)
  - existing_support: Added explicit column identification instructions
  - All other regions: Unchanged from v3

Outputs:
  data/test_outputs/phase6b_30pages_results_v3_1.json
  data/test_outputs/phase6b_30pages_extraction_summary_v3_1.csv
  data/test_outputs/phase6b_30pages_extraction_details_v3_1.csv
"""
...
Return ONLY this JSON:
{
  ...
  "AI": number or "uncertain" or null
    ...
    # ---- existing_support: v3.1 FIX — stronger template blacklist ----
    "existing_support": {
        "bounds": (12, 22, 65, 100),
        "zoom": 3,
        "items": ["HH", "HI", "HJ"],
...
This table has EXACTLY 3 columns per row:
  Column A (LEFT):   Item label text (e.g. "1. ネット追加", "2. 電話追加", "3. テレビ追加")
  Column B (MIDDLE): PRE-PRINTED TEMPLATE NUMBERS — these are FIXED values like 0, 15, 40
  Column C (RIGHT):  HANDWRITTEN ENTRY CELLS — this is the ONLY column you should read
...
They are NOT handwritten. NEVER return these as extracted values:
  - "0" in Column B → this is a printed zero, not a handwritten value
  - "40" in Column B → this is a printed template number for 電話追加
  - "15" in Column B → this is a printed template number

If you see "40" anywhere, it is ALWAYS a template value. Return null for that row.
If you see "0" in the MIDDLE column, it is a template value. Ignore it.

==== WHAT TO EXTRACT ====

Read ONLY Column C (rightmost cells) for handwritten entries:
- HH: Handwritten value in the RIGHT cell of row "1. ネット追加"
- HI: Handwritten value in the RIGHT cell of row "2. 電話追加"
- HJ: Handwritten value in the RIGHT cell of row "3. テレビ追加"

==== RULES ====

1. If the RIGHT cell is empty (no handwriting at all) → null
2. If staff clearly wrote a number in the RIGHT cell → return that number
3. Tally marks (正の字): 1 stroke=1, 2=2, 3=3, 4=4, complete 正=5. Unclear → null
4. Dashes (—) in RIGHT cell → 0
5. CRITICAL: The value "40" is NEVER a valid handwritten entry. Always return null if you would return 40.
6. Any printed/typeset number is NOT a handwritten value. Only ink that looks handwritten counts.
...
def main():
    print("=" * 70)
    print("Phase 6B v3.1: 30-Page Extraction (HI=40 Fix)")
    print("=" * 70)
    print()
    print("Key change from v3:")
    print("  existing_support: stronger template value blacklist (40 is NEVER valid)")
    print("  existing_support: explicit 3-column structure in prompt")
    print("  All other regions: unchanged from v3")
    print()
    ...
    summary_df.to_csv(OUTPUT_DIR / "phase6b_30pages_extraction_summary_v3_1.csv", index=False, encoding="utf-8-sig")
    print("\nSaved: phase6b_30pages_extraction_summary_v3_1.csv")

    details_df = pd.DataFrame(all_details)
    details_df.to_csv(OUTPUT_DIR / "phase6b_30pages_extraction_details_v3_1.csv", index=False, encoding="utf-8-sig")
    print("Saved: phase6b_30pages_extraction_details_v3_1.csv")
    ...
    results_json = {
        ...
        "version": "v3.1",
        "prompt_changes": [
            "existing_support: explicit 3-column structure (label/template/handwritten)",
            "existing_support: blacklisted template values (0, 15, 40) in prompt",
            "existing_support: rule that 40 is NEVER a valid handwritten entry",
            "All other regions unchanged from v3"
        ]
    }
    with open(OUTPUT_DIR / "phase6b_30pages_results_v3_1.json", "w", encoding="utf-8") as f:
        json.dump(results_json, f, ensure_ascii=False, indent=2)
    print("Saved: phase6b_30pages_results_v3_1.json")
