...
| 指標 | v3 | v5 | 結果 |
|------|-----|-----|------|
| 非null値 | 17/35 (48%) | 8/35 (23%) | v3優位 |
| 危険誤読（0 in tally） | 5 | 3 | 改善 ✓ |
| AI成功率 | 7/7 (100%) | 0/7 (0%) | 回帰 ❌ |

### 項目別結果

**AU**: v3 57% > v5 29% ❌
**AV**: v3 43% > v5 0% ❌（ただし危険誤読も0）
**AY**: v3 29% > v5 14% ❌
**AZ**: v3 14% < v5 71% ✅（大幅改善）
**AI**: v3 100% > v5 0% ❌（致命的回帰）

### 主な発見

1. **分割アプローチは理論的に正しい** — AZ改善で実証
2. **危険誤読は削減** — 案件欄の0が5→3に減少（40%削減）
3. **AI crop bounds が不適切** — 高さ 0.5% では Vision API が認識不可
4. **AV/AY の bounds も最適ではない** — 値が返されない

### 次のステップ

**v5.2 bounds最適化テスト** (現在実行中)

```
テスト設定:
  - AI row height: 0.5% → 1.5-2% に拡張
  - AV row bounds: (15.5, 19) → (15, 20) など複数試験
  - AY row bounds: (19, 21.5) → (18.5, 22.5) など複数試験

期待値:
  - AI がnullから回復 (0/7 → 5/7+を目指す)
  - AV が改善 (0/7 → 3/7+を目指す)
  - AZ改善を維持
```
...
**v5 最終判定**: **C. 不採用（実装品質が足りない）**

現在の v5 は採用できないが、v5.2 (bounds最適化版) に期待
