...
- **修正後CSV再取り込み / 再照合** → Salesforce側で修正したCSVを再度取り込んで再照合実行
  ...
  - 優先順：日報DataNo → タブレットNo → 日付＋店舗名＋担当者名
...
- **日報DataNo・タブレットNo** → PDF側の識別情報として保持するが、Salesforce CSVに存在しないため直接キーではない
- **集計値ベース照合** → 今回のサンプルでは日付・店舗コードを中心に照合（詳細は丸山様確認待ち）
- **再照合のキーベース比較** → 再照合時にPDFやCSVの行順が変わっても、比較キー（日報DataNo → タブレットNo → 日付+店舗+担当者）を使って正しく比較します。キーが作れない場合は「比較キー不足」として要確認に分類
