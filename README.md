# FAX帳票 × Salesforce CSV 照合確認システム（初期版）

FAX帳票PDFをSalesforce CSVと照合し、一致・不一致・要確認を整理するMVPアプリケーション
...
- **PDF一括アップロード** → 複数ファイル同時対応
- **Salesforce CSV取り込み** → 営業日で絞込可能
- **FAX帳票自動読取** → Claude Vision APIで日付・DataNo・TabNo・担当者名などを抽出
- **照合実行** → PDF結果 × Salesforce CSVを照合し「一致/不一致/要確認」を判定
- **対象日選択** → 営業日を選んでその日のCSVデータに絞込み
- **修正後CSV再取り込み / 再照合** → Salesforce側で修正したCSVを再度取り込んで再照合実行
  - **キーベース照合** 使用：PDF/CSVの行順が変わっても正しく比較
  - 比較キー：日付 ＋ 取扱店コード
- **確認ステータス管理**（初期版） → 各行に「未確認/確認済み/修正候補/再照合済み/要再確認」を記録
- **結果CSV出力** → 確認ステータスを含めてExport

### 初期版での前提・注意事項 ⚠️

- **Salesforce直接更新なし** → CSV出力をもとに人がSalesforce側で修正
- **Gmail送信・自動修正なし** → 人が確認前提の設計
- **確認ステータスは簡易機能** → 誰がいつ確認したかなどのログはSTAGE2で対応

- **照合単位（2026-06-02 電話確認反映）**
  - PDF帳票1ページ = Salesforce CSV 1行で対応
  - 複数ページの帳票は複数行のCSVと照合する

- **照合ロジック方針（2026-05-30/2026-06-02 片山様回答反映）**
  - Salesforce CSVには以下の項目を出力できない：
    - 日報DataNo（PDF帳票確認補助情報として保持、CSV直接照合キーには未使用）
    - タブレットNo（店舗確認補助情報として保持、スタッフ特定キーには非使用）
    - 担当者名（候補提示情報として保持、直接照合には非使用）
  - PDF側のこれらの項目は確認補助情報・参照情報のみとして扱う
  - 照合の主軸：**日付 + 取扱店コード**（PDF店舗名 × CSV取扱店コード対応表を作成予定）
  - 集計欄・商品別数字のCSV列対応は片山様からの回答待ち

- **スタッフ・店舗について（2026-06-02 電話確認反映）**
  - スタッフは店舗固定ではなく、スポット入店の場合がある
  - TabNoは**スタッフではなく店舗に紐づく**
  - 担当者特定時は店舗に所属するスタッフだけで限定してはいけない（スポット入店に対応できない）
  - 担当者名リストはジオ側で共有いただける可能性あり（STAGE 2で対応予定）

- **再照合のキーベース比較** → 再照合時にPDFやCSVの行順が変わっても、比較キー（日付 + 取扱店コード）を使って正しく比較します
