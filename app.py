    ...
    try:
        ...
        if len(df_filtered) > 0:
            with st.expander("💡 デモ用：代表ページの説明", expanded=False):
                special_pages = {
                    "P14": ("読み取り補正の例", "AIが大きく読み違えた値（34）を補正し、正しいCSV行（3）・PDF目視（3）と一致した例"),
                    "P16": ("ページ順比較による誤比較が解消した例", "店舗×日付キーで正しいCSV行を引くと、PDF目視（9）と完全一致。従来はページ順比較で別店舗の行と比較していた"),
                    "P30": ("ページ順比較による誤比較が解消した例", "店舗×日付キーで正しいCSV行を引くと、PDF目視（2）と完全一致。従来はページ順比較で別店舗の行と比較していた")
                }
